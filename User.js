...
function User()
{
  ...
  {
    querySql('SELECT pp_like.time,pp_like.work_id,pp_user.avatar,pp_user.nick_name FROM pp_like LEFT JOIN pp_work ON pp_work.id=pp_like.work_id LEFT JOIN pp_user ON pp_work.artist_id=pp_user.id WHERE pp_work.user_id='+ws.id+' ORDER BY pp_like.id DESC',function(err,result,field){

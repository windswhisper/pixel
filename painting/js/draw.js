...
var paintType = ORDER_TYPE_POINT;
var lineOx = 0;
var lineOy = 0;
...
window.onload=function(){
	...
	socket.onopen = function(event) { 
		...
		socket.send('{"event":1001,"index":16}');
	...
	$('body').on('mousedown touchstart', '#draw_canvas', function(e) {
		is_press = true;
		e=e||event;
		var x=(e.clientX||e.targetTouches[0].clientX)-c.offsetLeft;
		x=Math.floor(x/10);
		var y=(e.clientY||e.targetTouches[0].clientY)-c.offsetTop;
		y=Math.floor(y/10);
	...
	$('body').on('mouseup touchend', '#draw_canvas', function(e) {
		is_press = false;
		e=e||event;
		x=tempX;
		y=tempY;
		...
		if(paintType==ORDER_TYPE_PICKER)
			pickColor(x,y);
		sendOrderList();
		return false;
	});
	$('body').on('mousemove touchmove', '#draw_canvas', function(e)
	{  
		if(!is_press)return;
		e=e||event;
		var x=(e.clientX||e.targetTouches[0].clientX)-c.offsetLeft;
	...
	$("#straw_button").click (function()
	{
		paintType = ORDER_TYPE_PICKER;
	});
	...
	for(var i=0;i<8;i++)
	{
		var clickFunc = function(){
			setMainColor(arguments.callee.colorType);
		};
		clickFunc.colorType = i;
		$("#main_color_"+i).click(clickFunc);
	}

	for(var i=0;i<8;i++)
	{
		var clickFunc = function(){
			setSubColor(arguments.callee.colorType);
		};
		clickFunc.colorType = i;
		$("#sub_color_"+i).click(clickFunc);
	}
}


function setMainColor(id)
{
	colorTab = id;

	for(var i=0;i<8;i++)
	{
		var style = "background-color:"+COLOR[colorTab*8+i];
		if(i==0&&colorTab==0)
			style+=";box-shadow : 0 0 0 1px #e7e7e7;";
		$("#sub_color_"+i).attr("style",style);
	}
}
function setSubColor(id)
{
	color = colorTab*8+id;
}
